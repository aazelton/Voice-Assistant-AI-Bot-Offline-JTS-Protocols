#!/usr/bin/env python3
"""
Voice JTS Assistant - Hybrid Version
Works locally with option to connect to VM when available
"""
    ...
    jts_protocols = {
        ...
        'succinylcholine': {
            'rsi': {
                ...
                'cautions': 'Hyperkalemia, malignant hyperthermia risk',
        ...
        'etomidate': {
            'rsi': {
                ...
                'cautions': 'Adrenal suppression, myoclonus',
                'advantages': 'Minimal hemodynamic effects'
            }
        },
        'tranexamic acid': {
            'trauma': {
...
def get_ai_enhanced_response(question, weight=None, drug=None, calculated_dose=None, jts_info=None, conversation_history=None):
    ...
    try:
        ...
        SYSTEM_PROMPT = f"""You are a calm, experienced paramedic assisting another paramedic in the field. Your job is to provide direct, calculated answers without unnecessary information.

Context: {context_str}

CRITICAL RULES:
- Answer ONLY what was asked - no extra information
- If patient weight is provided, calculate and give TOTAL dose in mg/mcg
- NO cautions, contraindications, or warnings unless specifically asked
- Keep responses under 50 words
- Use simple bullet points
- If weight is known, convert mg/kg to total mg for the patient

Format responses as:
- Drug: X mg (total dose)
- Route: IV/IM

Example: If asked for ketamine RSI for 80kg patient, respond:
- Ketamine: 160 mg
- Route: IV

NOT: "Ketamine: 160 mg (2 mg/kg) - Caution: Monitor for hypertension..." """
...
def get_fallback_response(question, weight=None, drug=None, calculated_dose=None, jts_info=None):
    ...
    if weight and drug and calculated_dose:
        ...
        if '/kg' in unit:
            total_dose = dose * weight
            return f"{drug.title()}: {total_dose:.1f} {unit.replace('/kg', '')}"
        else:
            return f"{drug.title()}: {dose:.1f} {unit}"
    elif weight and drug:
        return f"Need indication for {drug} (RSI, pain, etc.)"
    elif jts_info:
        return f"{drug}: {jts_info.get('dose', 'Dose not specified')}"
    else:
        return "Need patient weight and medication."

def main():
    """Main voice assistant loop"""
    print("🎤 VOICE JTS ASSISTANT (HYBRID)")
    print("=" * 50)
    print("Voice + Local JTS Knowledge Base")
    print("📋 Note: For full JTS database, connect to VM when available")
    print("🎤 Initializing speech recognition...")
